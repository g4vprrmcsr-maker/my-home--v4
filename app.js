...
function paintTopbarTitle() {
  ...
  if (isKv) {
    if (menuBtn) menuBtn.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7h11"/><path d="M4 12h16"/><path d="M4 17h8"/></svg>';
    ...
    if (kvMb) {
      ...
      if (svg) {
        const w = el("span", "");
        w.style.cssText = "display:inline-flex;align-items:center;justify-content:center;width:29px;height:29px;color:var(--text-main);pointer-events:none;";
        w.innerHTML = svg;
        const si = w.querySelector("svg");
        if (si) { si.style.width = "25px"; si.style.height = "25px"; si.style.pointerEvents = "none"; }
        kvMb.appendChild(w);
      } else {
